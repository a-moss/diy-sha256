...
function ror(value, shiftAmount) {
    ...
    const rotatedBits = (((parsedValue >>> shiftAmount) | (parsedValue << mask)) >>> 0).toString(2);
...
function entry() {
    ...
    let [h0, h1, h2, h3, h4, h5, h6, h7,] = generateConstants(calculatePrimeNumbers(8), Math.sqrt);
    ...
    for (let i = 0; i < words.length; i++) {
        ...
        const ch = xor((e & f), (~e & g));
        const temp1 = h + s1 + ch + kConstants[i] + words[i]; // TODO: don't forget to do mod2^32 arithmetic
        const s0Xor1 = xor(ror(a, 2), ror(a, 13));
        const s0 = xor(s0Xor1, ror(a, 22));
        const majXor1 = xor((a & b), (a & c));
        const maj = xor(majXor1, (b & c));
        const temp2 = s0 + maj;
        h = g;
        g = f;
        f = e;
